# Checklist รัน Lab 0–2 บน Fabric UI (ทีละเซลล์)

ใช้คู่กับ workspace **`labs`** → Lakehouse **`lh_freshmart`**  
...
| 3 | **รันโค้ด** โหลด | อ่าน transactions + customers | `Transactions rows: 3,000` และ `Customers rows: 1,500` | fallback CSV จาก `Files/raw` ได้ |
| 4 | markdown | อ่านจุดตรวจ | — | — |
| 5 | **รันโค้ด** จุดตรวจ | ตรวจจำนวนแถว | **`Lab 0 verification passed`** | อ่าน AssertionError |
...
| # | เซลล์ | ทำอะไร | ต้องเห็น | ถ้าติด |
| --- | --- | --- | --- | --- |
| 0 | markdown | อ่านบทบาท Analyst | — | — |
| 1 | **รันโค้ด** loader | นิยาม loader | ไม่ error | แนบ lakehouse |
| 2–3 | markdown + **รัน** | โหลดธุรกรรม | shape `(3000, 14)` | กลับไป Lab 0 |
| 4–5 | markdown + **รัน** | Data quality | `DiscountRate` ว่าง ~89 แถว (~3%) | — |
| 6–7 | markdown + **รัน** | describe | ตารางสถิติ 6 คอลัมน์ | — |
| 8–9 | markdown + **รัน** | histogram | Waste เบ้ขวา | รอ cold start |
| 10 | **รันโค้ด** | box plot ตาม StoreType | Express สูงกว่า Hypermarket โดยประมาณ | — |
| 11–12 | markdown + **รัน** | correlation | DiscountRate↔UnitsSold ≈ +0.38 | — |
| 13–14 | markdown + **รัน** | churn EDA | 1,500 คน, Churn ~19.3%, Age ว่าง 37 | — |
| 15 | **รันโค้ด** จุดตรวจ | assert สุดท้าย | **`Lab 1 verification passed`** | อ่าน error |

**จด 3 ข้อส่ง Lab 2:** เติม Age · แปลงหมวดหมู่ · ปรับสเกลตัวเลขเงิน/ความถี่
...
- [ ] แท็บ **Home** → **Data Wrangler** → เลือก `df`
...
| — | **รันโค้ด** | transform | Age median **37.0**, one-hot ครบ, สเกล ~0–1 | — |
